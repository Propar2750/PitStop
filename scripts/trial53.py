"""Trial 53 (RealMLP architecture variant for decorrelation): RealMLP (PyTorch), trial45 recipe under our baseline CV.
...
# Save OOF + submission
np.save("/tmp/trial53_oof.npy", oof_preds)
np.save("/tmp/trial53_test.npy", test_preds)
sub = pd.DataFrame({ID: test_id, TARGET: test_preds})
sub.to_csv(DATA_DIR / "submission_trial53.csv", index=False)
print("Wrote submission_trial53.csv")
