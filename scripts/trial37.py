"""Trial 37: Transformer v3 — concat categoricals, deeper, simple recipe.
...
Ensemble: rank-avg(T35 XGB, T35 CB) + LR-meta with T37_TFM logits;
...
print(f'\nT37 TFM OOF AUC = {tfm_auc:.5f}  ({time.time()-t_tfm:.0f}s total)', flush=True)
np.save('/tmp/trial37_tfm_oof.npy',  tfm_oof.astype(np.float32))
np.save('/tmp/trial37_tfm_test.npy', tfm_test.astype(np.float32))
...
print(f'LR-meta(GBDT, T37_TFM) OOF AUC = {meta_auc:.5f}', flush=True)
...
print(f'\ncorr(T37_TFM, XGB) = {corr_xgb:.4f}', flush=True)
print(f'corr(T37_TFM, CB ) = {corr_cb:.4f}', flush=True)

print(f'\n==== T37 RESULTS ====', flush=True)
print(f'T37 TFM   OOF AUC : {tfm_auc:.5f}  (T35 TFM 0.93542 -> delta {tfm_auc-0.93542:+.5f})', flush=True)
