"""Trial 55 (iterative pseudo-labeling from T54-7way): RealMLP (PyTorch), trial45 recipe under our baseline CV.
...
PSEUDO_THR = 0.80
best7way_test = np.load("/tmp/best7way_v2_test.npy")
...
for fold, (tr_idx, val_idx) in enumerate(skf.split(X, y, groups=groups), 1):
    ...
    model.fit(
        ...
        ckpt_path=f"/tmp/realmlp55_fold{fold}.pth",
    )
...
# Save OOF + submission
np.save("/tmp/trial55_oof.npy", oof_preds)
np.save("/tmp/trial55_test.npy", test_preds)
sub = pd.DataFrame({ID: test_id, TARGET: test_preds})
sub.to_csv(DATA_DIR / "submission_trial55.csv", index=False)
print("Wrote submission_trial55.csv")
