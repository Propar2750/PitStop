...
# Load T25 ensemble predictions (LR-meta of XGB-2 + CB-4) as the pseudo-label source
oof_xgb_2 = np.load('/tmp/trial23_oof_xgb.npy')
oof_cb_4  = np.load('/tmp/trial25_oof_cb.npy')
test_xgb_2 = np.load('/tmp/trial23_test_xgb.npy')
test_cb_4  = np.load('/tmp/trial25_test_cb.npy')
...
XGB_SEEDS = [42, 2024]
CB_SEEDS  = [42, 2024, 7, 31337]
SEEDS = XGB_SEEDS  # legacy var (XGB still uses 2 seeds)
N_SPLITS = 5
...
for fold, (tr_idx, va_idx) in enumerate(cv.split(X_train_real, y_real, groups)):
    ...
    for s in XGB_SEEDS:
        ...
        fold_xgb_va += m.predict_proba(X_va_xgb)[:, 1] / len(XGB_SEEDS)
        fold_xgb_te += m.predict_proba(X_test_xgb_full)[:, 1] / len(XGB_SEEDS)
    ...
    for s in CB_SEEDS:
        ...
        fold_cb_va += cm.predict_proba(X_va)[:, 1] / len(CB_SEEDS)
        fold_cb_te += cm.predict_proba(X_test_full)[:, 1] / len(CB_SEEDS)
...
np.save('/tmp/trial29_oof_xgb.npy', oof_xgb)
np.save('/tmp/trial29_oof_cb.npy',  oof_cb)
np.save('/tmp/trial29_test_xgb.npy', test_xgb_new)
np.save('/tmp/trial29_test_cb.npy',  test_cb_new)
