"""Trial 39: T37 + rare-driver bucketing + larger Driver embedding.

Single change vs T37: drivers appearing in fewer than MIN_DRIVER_COUNT
sequences across train+test get collapsed into a single "<OOV>" category.
The Driver embedding dim is bumped 24 -> 40 to give the now-cleaner
high-frequency drivers more capacity.

Rationale: 887 driver vocab in T37, but pit-stop behavior for a driver
with 1-2 races worth of laps cannot be learned from those few examples.
Pooling them into an OOV bucket means the model defaults to a stable
"unknown driver" prior for those rows, while the well-supported drivers
get a larger embedding.
"""
...
MIN_DRIVER_COUNT = 5   # sequences threshold for "well-supported"
...
# Driver bucketing: count sequences per driver, OOV anyone with <MIN_DRIVER_COUNT.
seq_counts = (df.groupby(['Year','Race','Driver']).size().reset_index()
              .groupby('Driver').size())
keep_drivers = set(seq_counts[seq_counts >= MIN_DRIVER_COUNT].index)
n_total = df['Driver'].nunique()
n_keep  = len(keep_drivers)
df['Driver'] = df['Driver'].where(df['Driver'].isin(keep_drivers), '<OOV>')
print(f'Driver vocab: {n_total} total -> {n_keep} kept ({n_total-n_keep} bucketed into <OOV>)', flush=True)
...
class PitTransformerV3(nn.Module):
    def __init__(self, num_cols, cat_vocab,
        ...
        super().__init__()
        if cat_dims is None:
            cat_dims = {'Driver': 40, 'Compound': 6, 'Race': 12}   # Driver bumped 24 -> 40
...
t_tfm = time.time()
print('\n=== T39: T37 + rare-driver bucketing + Driver dim 24->40 ===', flush=True)
...
print(f'\nT39 TFM OOF AUC = {tfm_auc:.5f}  (T37 0.93958 -> delta {tfm_auc-0.93958:+.5f})', flush=True)
np.save('/tmp/trial39_tfm_oof.npy',  tfm_oof.astype(np.float32))
np.save('/tmp/trial39_tfm_test.npy', tfm_test.astype(np.float32))
...
print(f'\n==== T39 RESULTS ====', flush=True)
print(f'T39 TFM OOF AUC : {tfm_auc:.5f}', flush=True)
...
print(f'corr(T39_TFM, XGB)={np.corrcoef(xgb_oof, tfm_oof)[0,1]:.4f} '
      f'corr(T39_TFM, CB)={np.corrcoef(cb_oof, tfm_oof)[0,1]:.4f}', flush=True)
