"""Trial 43: T42 + LapNumber as numeric + aux head (next-lap TyreLife).

Goal: push the transformer's standalone OOF past 0.945. T42 already
delivered 0.94379 from engineered features alone; T43 bundles two
additional proven wins:

  1. LapNumber added back to the numeric feature set (was dropped per
     baseline rule). T8 isolated this as +0.0015 for LightGBM.
  2. Auxiliary head predicting next-lap TyreLife (MSE, w=0.3).
     T40 isolated this as +0.0003 for the raw-feature transformer.

Bundling so we cross 0.945 in one trial rather than two.

Everything else: T42 setup unchanged (concat fusion, d=128, 4 layers,
6-seed bag, T35 simple recipe). Reuses T35 XGB/CB OOFs for ensemble.
"""
...
# +LapNumber vs T42.
RAW_NUM = ['Year','LapNumber','PitStop','Stint','TyreLife','Position','LapTime (s)',
...
print(f'numerics: {len(NUM_COLS)} ({len(RAW_NUM)} raw incl. LapNumber + {len(ENG_NUM)} engineered)', flush=True)
...
t_tfm = time.time()
print('\n=== T43: T42 + LapNumber + aux head ===', flush=True)
...
print(f'\nT43 TFM OOF AUC = {tfm_auc:.5f}  (T42 0.94379 -> delta {tfm_auc-0.94379:+.5f})', flush=True)
np.save('/tmp/trial43_tfm_oof.npy',  tfm_oof.astype(np.float32))
np.save('/tmp/trial43_tfm_test.npy', tfm_test.astype(np.float32))
...
print(f'\n==== T43 RESULTS ====', flush=True)
print(f'T43 TFM OOF AUC : {tfm_auc:.5f}', flush=True)
...
print(f'corr(T43, XGB)={np.corrcoef(xgb_oof, tfm_oof)[0,1]:.4f} '
      f'corr(T43, CB )={np.corrcoef(cb_oof, tfm_oof)[0,1]:.4f}', flush=True)
