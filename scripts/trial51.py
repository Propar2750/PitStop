"""Trial 51 (RealMLP, 8 epochs vs 4): RealMLP (PyTorch), trial45 recipe under our baseline CV.
...
# Save OOF + submission
np.save("/tmp/trial51_oof.npy", oof_preds)
np.save("/tmp/trial51_test.npy", test_preds)
sub = pd.DataFrame({ID: test_id, TARGET: test_preds})
sub.to_csv(DATA_DIR / "submission_trial51.csv", index=False)
print("Wrote submission_trial51.csv")
