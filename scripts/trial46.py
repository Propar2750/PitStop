...
# Save OOF + submission
np.save("/tmp/trial46_oof.npy", oof_preds)
np.save("/tmp/trial46_test.npy", test_preds)
sub = pd.DataFrame({ID: test_id, TARGET: test_preds})
sub.to_csv(DATA_DIR / "submission_trial46.csv", index=False)
print("Wrote submission_trial46.csv")
