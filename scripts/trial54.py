...
for fold, (tr_idx, val_idx) in enumerate(skf.split(X, y, groups=groups), 1):
    ...
    model.fit(
        ...
        ckpt_path=f"/tmp/realmlp54_fold{fold}.pth",
    )
...
# Save OOF + submission
np.save("/tmp/trial54_oof.npy", oof_preds)
np.save("/tmp/trial54_test.npy", test_preds)
sub = pd.DataFrame({ID: test_id, TARGET: test_preds})
sub.to_csv(DATA_DIR / "submission_trial54.csv", index=False)
print("Wrote submission_trial54.csv")
