"""Trial 45: RealMLP (PyTorch) replication of the Kaggle notebook
``yekenot/ps-s6-e5-realmlp-pytorch``.

Faithful reproduction. Key deviations from our baseline (called out so the
score is not naively compared against trials 1-44):

  * CV scheme is **StratifiedKFold** on the row level (NOT
    StratifiedGroupKFold by Race_Year_Driver). This means stints from the
    same (Race, Year, Driver) can leak across folds; OOF scores will be
    optimistic vs. our group-stratified trial log.

  * **External data**: 101371 rows from the Kaggle dataset
    ``aadigupta1601/f1-strategy-dataset-pit-stop-prediction`` (cached at
    ``~/.cache/kagglehub/...``) are concatenated to the training fold each
    iteration. None of the prior trials used external data.

  * Feature engineering, preprocessing, model architecture, hyperparams,
    optimizer/scheduler config, and CONFIG dict are copied verbatim from the
    notebook (RealMLP with PBLD numerical embeddings, NTPLinear ensemble of
    16 inner members, AdamW with per-group LRs, label-smoothed BCE with
    class weights, flat-cos LR schedule, 4 epochs, batch=256, GPU).

Outputs:
  - prints fold AUCs and overall OOF AUC
  - writes ``submission_trial45.csv`` (column ``PitNextLap``)
"""
import math
import random
import time
...
ORIG_CSV = Path.home() / ".cache/kagglehub/datasets/aadigupta1601/f1-strategy-dataset-pit-stop-prediction/versions/4/f1_strategy_dataset_v4.csv"
...
orig, *_ = feature_engineering(orig, fit=False)
...
for fold, ((tr_idx, val_idx), (or_tr_idx, or_val_idx)) in enumerate(
    ...
    model.fit(
        ...
        ckpt_path=f"/tmp/realmlp_fold{fold}.pth",
    )
...
# Save OOF + submission
np.save("/tmp/trial45_oof.npy", oof_preds)
np.save("/tmp/trial45_test.npy", test_preds)
sub = pd.DataFrame({ID: test_id, TARGET: test_preds})
sub.to_csv(DATA_DIR / "submission_trial45.csv", index=False)
print("Wrote submission_trial45.csv")
