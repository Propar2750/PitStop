"""Trial 56 (T54 pseudo recipe, seed=2024 for variance reduction): RealMLP (PyTorch), trial45 recipe under our baseline CV.
...
# ─── Pseudo-labels from T48 7-way LR-meta ────────────────────────────────────
PSEUDO_THR = 0.80
best7way_test = np.load("/tmp/best7way_test.npy")
pseudo_mask = best7way_test > PSEUDO_THR
n_pseudo = int(pseudo_mask.sum())
print(f"\nPseudo-labels: {n_pseudo} rows ({100*n_pseudo/len(best7way_test):.1f}% of test) "
      f"with pred>{PSEUDO_THR} from T48 7-way ensemble", flush=True)
X_pseudo = X_test.iloc[pseudo_mask].copy().reset_index(drop=True)
y_pseudo = pd.Series(np.ones(n_pseudo, dtype=np.int64), name=TARGET)
...
for fold, (tr_idx, val_idx) in enumerate(skf.split(X, y, groups=groups), 1):
    ...
    # Append pseudo-positives to training fold
    X_tr = pd.concat([X_tr, X_pseudo], axis=0).reset_index(drop=True)
    y_tr = pd.concat([y_tr, y_pseudo], axis=0).reset_index(drop=True)
    # Re-cast categoricals after concat (pd.concat downcasts cat dtype)
    for c in cat_cols:
    ...
    model.fit(
        ...
        ckpt_path=f"/tmp/realmlp56_fold{fold}.pth",
    )
...
# Save OOF + submission
np.save("/tmp/trial56_oof.npy", oof_preds)
np.save("/tmp/trial56_test.npy", test_preds)
sub = pd.DataFrame({ID: test_id, TARGET: test_preds})
sub.to_csv(DATA_DIR / "submission_trial56.csv", index=False)
print("Wrote submission_trial56.csv")
