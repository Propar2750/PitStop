"""Trial 31: per-driver-race transformer (causal-masked).

Input: full (Year, Race, Driver) lap sequence (train + test rows together).
Each lap = numeric features + sum of (Driver, Compound, Race, Year) embeddings.
Causal mask: lap t attends only to <=t. Predict per-lap PitNextLap.
Train on train-row labels only (test rows = unlabeled context).
...
np.save('/tmp/trial31_oof.npy', oof_train)
np.save('/tmp/trial31_test.npy', test_pred)
